...
mod tests {
    ...
    #[test]
    fn test_ignition_fragment_cmd_rejects_platform_arg() {
        let args: Vec<_> = ["afterburn", "ignition-fragment", "--platform", "azure"]
            .iter()
            .map(ToString::to_string)
            .collect();

        parse_args(args).expect_err("platform arg should be rejected");
    }
}
