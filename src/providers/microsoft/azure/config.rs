...
//!
//! Mounts the OVF provisioning media, reads user/SSH config from OVF + IMDS,
//! and writes a JSON Ignition fragment to
...
pub(crate) fn generate() -> Result<()> {
    ...
    let imds_username = fetch_os_profile_username(&imds)?;
    let ssh_keys = fetch_imds_ssh_keys(&imds)?;

    let ovf_username = read_ovf_username()?;

    let username = match ovf_username {
        Some(u) => u,
        None => {
            info!("using username from IMDS osProfile");
            imds_username
        }
    };
...
/// Returns `Ok(None)` if OVF media isn't available.
fn read_ovf_username() -> Result<Option<String>> {
...
fn parse_ovf_env(xml: &str) -> Result<OvfEnvironment> {
    ...
    if !env
        ...
        .is_empty()
    {
        anyhow::bail!("OVF contains a non-empty password, which is not supported");
    }
    Ok(env)
}
...
<Environment xmlns="http://schemas.dmtf.org/ovf/environment/1"
    ...
    <wa:ProvisioningSection>
        ...
        assert_eq!(
            env.provisioning_section.linux_prov_conf_set.username,
            "myuser"
        );
    }

    #[test]
    fn test_ovf_rejects_password() {
        let xml = r#"
<Environment xmlns="http://schemas.dmtf.org/ovf/environment/1"
    xmlns:wa="http://schemas.microsoft.com/windowsazure">
    <wa:ProvisioningSection>
        ...
        assert!(parse_ovf_env(xml).is_err());
    }
